...
function create(topic, options) {
    ...
    switch (topic) {
        case 'component':
            ...
            target = options.output + '\\scripts\\tau\\components';
            output = [target];
            var test = /Tp\.Web.JavaScript.tau/.test(target);
            var testsFilePath = options.output + '\\scripts\\tests.async\\tests.js';
            var testsFile = fs.readFileSync(testsFilePath, {encoding: 'utf-8'});
            var ast = UglifyJS.parse(testsFile);
            var rec = new UglifyJS.AST_String({value: 'tests.async/components/' + options.instanceName + '/component.' + options.instanceName + '.tests'});
            ast.body[0].body.args[0].body[0].value.elements.push(rec);
            ...
            ast.print(out);
            var str = esformatter.format(out.toString(), optionsFormatter);
            fs.writeFileSync(testsFilePath, str);
            createTestsFilesStep();
            if (!test && !options.ignorePath) {
                console.warn('You should run script from Code/Main/Tp.Web/JavaScript/tau/');
                process.exit();
            }
    ...
    function createTestsFilesStep() {
        // create file structure

        createFiles(options.templates[0] + '/tests/', [options.output + '/scripts/tests.async/components/'], {
            name: options.name,
            instanceName: options.instanceName
        });
    }
